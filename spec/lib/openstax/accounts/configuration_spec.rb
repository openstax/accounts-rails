module OpenStax
  module Accounts
    RSpec.describe Configuration do
      let!(:config) do
        Configuration.new.tap {|c| c.openstax_accounts_url = "https://openstax.org/accounts"}
      end
      let!(:a_fake_request) { OpenStruct.new(url: "http://foo.com") }

      it "returns the default logout redirect when no explicit URL is set" do
        expect(config.logout_redirect_url(a_fake_request)).to(
          eq "https://openstax.org/accounts/logout"
        )
      end
